...
from schedule_refactor.models.rules import Rule
from schedule_refactor.models.rooms import Room
from schedule_refactor.utils import OccurrenceReplacer
...
class Reservation(models.Model):
    ...
    rule = models.ForeignKey(Rule, null=True, blank=True, verbose_name=_("rule"),
                             help_text=_("Select '----' for a one time only reservation."))
    end_recurring_period = models.DateTimeField(_("end recurring period"), null=True, blank=True,
                                                help_text=_("This date is ignored for one time only reservations."))
...
class ReservationRelationManager(models.Manager):
    ...
    def get_reservations_for_object(self, content_object, distinction=None, inherit=True):
        ...
        if inherit:
            ...
            inherit_q = Q()
        reservation_q = Q(dist_q, Q(reservationrelation__object_id=content_object.id),
                          Q(reservationrelation__content_type=ct))
